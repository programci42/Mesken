...
function showNotification(user, message) {
    // Tarayıcı bildirimini göster (izin varsa)
    if ('Notification' in window && Notification.permission === 'granted') {
        const browserNotification = new Notification(`${user.nickname} size mesaj gönderdi`, {
            body: message,
            icon: user.profile_picture || 'default-icon.png',
            tag: 'message-notification',
            requireInteraction: true, // Bildirimin otomatik olarak kapanmasını engeller
            silent: false // Tarayıcı varsayılan sesini kullan
        });
        
        // Bildirime tıklandığında sohbeti aç
        ...
        Notification.requestPermission().then(permission => {
            if (permission === 'granted') {
                showNotification(user, message); // İzin alındıktan sonra tekrar çağır
            }
        });
    }
    ...
    notification.dataset.userId = user.id; // Kullanıcı ID'sini veri olarak ekle
    
    // Mobil cihazlarda titreşim (destekleniyorsa)
    if ('vibrate' in navigator) {
        navigator.vibrate([200, 100, 200]); // Titreşim deseni: 200ms açık, 100ms kapalı, 200ms açık
    }
    ...
    closeBtn.innerHTML = '✕';
    closeBtn.title = 'Bildirimi kapat';
    closeBtn.onclick = (e) => {
        e.stopPropagation();
        notification.style.animation = 'slideIn 0.3s ease reverse';
        setTimeout(() => notification.remove(), 300);
    ...
    unreadCount.className = 'notification-unread-count';
    
    // Bu kullanıcı için kaç okunmamış mesaj olduğunu say
    const existingNotifications = container.querySelectorAll(`.notification[data-user-id="${user.id}"]`);
    unreadCount.textContent = existingNotifications.length + 1;

    content.appendChild(title);
    content.appendChild(messageText);
    notification.appendChild(avatar);
    notification.appendChild(content);
    notification.appendChild(unreadCount);
    notification.appendChild(markReadBtn);
    ...
    // Daha dikkat çekici sesli uyarı
    playNotificationSound();

    // Daha uzun süre göster ve daha belirgin animasyon ile kapat
    setTimeout(() => {
        notification.style.animation = 'notificationPulse 0.5s ease reverse';
        setTimeout(() => {
            notification.style.animation = 'slideIn 0.3s ease reverse';
            setTimeout(() => notification.remove(), 300);
        }, 500);
    }, 8000); // 8 saniye göster (önceki 5 saniyeden artırıldı)
}
...
function playNotificationSound() {
    try {
        ...
        const totalDuration = 2.0; // Toplam ses süresini 2 saniyeye çıkardık

        // Ana ses seviyesini kontrol etmek için bir master gain node
        const masterGainNode = audioContext.createGain();
        masterGainNode.connect(audioContext.destination);
        masterGainNode.gain.setValueAtTime(0.8, now); // Yüksek başlangıç sesi
        masterGainNode.gain.exponentialRampToValueAtTime(0.01, now + totalDuration); // Yavaşça azalır

        // --- 1. Kısım: Kalınlaştırıcı "Vurmuş" Etkisi ---
        // Sese daha fazla ağırlık katmak için çok düşük frekanslı bir ton
        const thumpOsc = audioContext.createOscillator();
        const thumpGain = audioContext.createGain();
        thumpOsc.type = 'sine'; // Yumuşak bir başlangıç için
        thumpOsc.frequency.setValueAtTime(70, now); // Çok düşük frekans (vurmuş efekti)
        thumpOsc.connect(thumpGain);
        thumpGain.connect(masterGainNode);
        
        // Vurmuş efekti: Hızlı yükselir ve hızlı söner
        thumpGain.gain.setValueAtTime(0, now);
        thumpGain.gain.linearRampToValueAtTime(1.0, now + 0.02); // Çok hızlı başlangıç
        thumpGain.gain.exponentialRampToValueAtTime(0.01, now + 0.2); // 200ms'de söner
        thumpOsc.start(now);
        thumpOsc.stop(now + 0.25);

        // --- 2. Kısım: Ana Uyarı Sinyali (Daha Kalın ve Uzun) ---
        // Kare dalga formu kullanarak daha keskin ve kalın bir ton oluşturalım
        const frequencies = [350, 500, 700]; // Daha düşük ve kalın frekanslar
        const startTimes = [0.2, 0.8, 1.4]; // Tonların başlama zamanları (uzatılmış aralıklar)
        const durations = [0.6, 0.6, 0.6];   // Her bir tonun süresi

        frequencies.forEach((freq, index) => {
            const osc = audioContext.createOscillator();
            const gainNode = audioContext.createGain();

            osc.type = 'square'; // Kalın ve keskin bir ton için kare dalga
            osc.frequency.setValueAtTime(freq, now + startTimes[index]);

            osc.connect(gainNode);
            gainNode.connect(masterGainNode);

            // Her tonun başında ve sonunda "klik" seslerini önlemek için yumuşatma
            gainNode.gain.setValueAtTime(0, now + startTimes[index]);
            gainNode.gain.linearRampToValueAtTime(0.6, now + startTimes[index] + 0.05); // Hızlı yükselme
            gainNode.gain.linearRampToValueAtTime(0.6, now + startTimes[index] + durations[index] - 0.05); // Sürdürme
            gainNode.gain.linearRampToValueAtTime(0, now + startTimes[index] + durations[index]); // Hızlı düşme

            osc.start(now + startTimes[index]);
            osc.stop(now + startTimes[index] + durations[index]);
        });

        // Tarayıcının otomatik ses oynatma kısıtlamasını aşmak için
...
function togglePanel() 
{
    ...
    const isMobile = window.innerWidth <= 768;
		...
		{
            icon.textContent = '✕';
            toggleBtn.style.left = ofsetwidth;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const icon = document.getElementById('toggleIcon');
    const toggleBtn = document.getElementById('panelToggleBtn');
    ofsetwidth = (parseInt(getComputedStyle(panel).width) - 30).toString() + "px";
